...
export function Cta({ block }: CtaProps): JSX.Element {
  ...
  return (
    <section>
      <Typography className={styles['title']} label={block.item} fontFamily="helvetica-medium" />
        ...
        {block.items.map((item, key) => (
          <div className={styles['container-item']} key={key}>
            ...
            <div className={styles['content']}>
              <Typography label={item.title} fontFamily="helvetica-medium" />
              <Typography label={item.description} fontFamily="helvetica-regular" />
              <Typography className={styles['cta']} label={item.cta} fontFamily="helvetica-light" />
            </div>
          </div>
        ))}
      </div>
